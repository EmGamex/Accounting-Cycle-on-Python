...
class TestModularidadOperaciones(unittest.TestCase):
    ...
    def test_reordenar_partidas_posicion(self):
        """Verifica mover partidas a nuevas posiciones correlativas."""
        gestor = GestorLibroDiario()
        ...
        p3 = PartidaDiario(3, date(2026, 1, 3), "Asiento 3", [
            MovimientoLinea("1101", "Caja", debe=Decimal("30.00")),
            MovimientoLinea("1102", "Bancos", haber=Decimal("30.00")),
        ])
        gestor.registrar_partida(p1)
        gestor.registrar_partida(p2)
        gestor.registrar_partida(p3)

        # Mover partida #3 a la posición #1
        self.assertTrue(gestor.mover_partida(3, 1))
        self.assertEqual(gestor.libro.partidas[0].numero, 1)
        self.assertEqual(gestor.libro.partidas[0].glosa, "Asiento 3")
        self.assertEqual(gestor.libro.partidas[1].numero, 2)
        self.assertEqual(gestor.libro.partidas[1].glosa, "Asiento 1")
        self.assertEqual(gestor.libro.partidas[2].numero, 3)
        self.assertEqual(gestor.libro.partidas[2].glosa, "Asiento 2")

        # Mover a la misma posición retorna True
        self.assertTrue(gestor.mover_partida(1, 1))
        # Rango inválido
        self.assertFalse(gestor.mover_partida(0, 2))
        self.assertFalse(gestor.mover_partida(1, 99))

    def test_ordenar_partidas_cronologicamente(self):
        """Verifica que ordenar cronológicamente indexe de menor a mayor fecha con correlativos 1..N."""
        gestor = GestorLibroDiario()
        p1 = PartidaDiario(1, date(2026, 1, 15), "Compra Quincena", [
            MovimientoLinea("1101", "Caja", debe=Decimal("10.00")),
            MovimientoLinea("1102", "Bancos", haber=Decimal("10.00")),
        ])
        p2 = PartidaDiario(2, date(2026, 1, 2), "Apertura Banco", [
            MovimientoLinea("1101", "Caja", debe=Decimal("20.00")),
            MovimientoLinea("1102", "Bancos", haber=Decimal("20.00")),
        ])
        p3 = PartidaDiario(3, date(2026, 1, 8), "Venta Semana 1", [
            MovimientoLinea("1101", "Caja", debe=Decimal("30.00")),
            MovimientoLinea("1102", "Bancos", haber=Decimal("30.00")),
        ])
        gestor.registrar_partida(p1)
        ...
        self.assertEqual(gestor.libro.partidas[0].numero, 1)
        self.assertEqual(gestor.libro.partidas[0].glosa, "Apertura Banco")
        self.assertEqual(gestor.libro.partidas[0].fecha, date(2026, 1, 2))

        self.assertEqual(gestor.libro.partidas[1].numero, 2)
        self.assertEqual(gestor.libro.partidas[1].glosa, "Venta Semana 1")
        self.assertEqual(gestor.libro.partidas[1].fecha, date(2026, 1, 8))

        self.assertEqual(gestor.libro.partidas[2].numero, 3)
        self.assertEqual(gestor.libro.partidas[2].glosa, "Compra Quincena")
        self.assertEqual(gestor.libro.partidas[2].fecha, date(2026, 1, 15))
